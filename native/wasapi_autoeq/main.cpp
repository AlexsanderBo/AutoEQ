...
constexpr REFERENCE_TIME kStableBufferDuration100ns = 10000000; // 100ms stable fallback for shared WASAPI.
constexpr REFERENCE_TIME kLowLatencyBufferDuration100ns = 200000; // 20ms low-latency target for shared WASAPI.
...
class WasapiLoopbackAnalyzer {
public:
    ~WasapiLoopbackAnalyzer() { cleanup(); }
    ...
    bool initialize(bool enableRender, const std::wstring& inputHint = L"", const std::wstring& outputHint = L"", LatencyProfile latencyProfile = LatencyProfile::Low) {
        ...
        logEndpointInfo(captureDeviceName_);
...
private:
    ...
    void logEndpointInfo(const std::wstring& name) {
        std::cout << "[Device] Analyzer endpoint ready: " << narrow(name) << std::endl;
    }
    ...
    bool configureFormat(IAudioClient* client) {
        ...
        if (hr == S_OK) {
            ...
            std::cout << "[Format] Using preferred shared WASAPI format: 48000 Hz, stereo, IEEE float." << std::endl;
            return true;
        }
    ...
    void printConfiguration() const {
        std::cout << "[Config] Hardware target: default Windows audio endpoint" << std::endl;
        std::cout << "[Config] Codec target: endpoint mix format selected by WASAPI" << std::endl;
...
public:
    ...
    int run(int seconds) {
        HANDLE mutex = CreateMutexW(nullptr, TRUE, L"Global\\AutoEQ_WASAPI_ANALYZER");
...
private:
    ...
    WasapiLoopbackAnalyzer wasapi_;
...
void printUsage() {
    std::cout << "WASAPI AutoEQ native analyzer\n"
              << "Usage:\n"
              << "  wasapi_autoeq.exe --analyze [--latency low|stable] [--seconds N]\n"
              << "  wasapi_autoeq.exe --process --input \"Virtual Cable\" --output \"Speakers\" [--latency low|stable] [--seconds N]\n\n"
              << "Modes:\n"
